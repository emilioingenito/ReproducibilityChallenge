...
# main function which allows to run the text classification task
# specifying some crucial parameters, such as the pretrained embeddings, 
# the model, the vocabulary, the learning rate, the epochs, the dataset
# (splitted into train, dev, and test set) etc.
...
# computes the epoch loss and the epoch accuracy
...
# computes the epoch time, given the starting time and the ending time
...
# loads a vocabulary, given its path and the index of padding
...
# generates the padding for a given sentence
...
# load a sentence by its id, given input data
...
# loads all the embedding of a given file, given the file path and 
# the embedding size
...
# gets the weights of the embeddings, given a list of word
# and the embedding size, which will be 300 for fast_text + LOVE
# and 768 for BERT + LOVE
...
# processes the embedding and writes the results in an output file
...
# load pretrained embedding given the embedding size
# which will be 300 for fast_text + LOVE and 768 for BERT + LOVE
...
# runs the analysis for the vdataset SST2
def run_sst2(
        args,
):
...
# prints out the results of the analysis on the test set
